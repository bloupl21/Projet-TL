...
def test_rattrapage_error(calc_input, expected):
    print("@ test {0} on input:".format(PARSER_NAME), repr(calc_input))
    print("@ rattrapage error expected")
    try:
        result=run(calc_input)
        print("@ unexpected result:", result)
        exit(1)
    except ParserError as e:
        print("@ rattrapage error found:", e)
        pass
    print("@ => OK")
    print()
...
# Tests avec erreurs
test_rattrapage_error(";",[0])
test_rattrapage_error("123+321",[444])
test_rattrapage_error("123+321; 1",[444,1])
test_rattrapage_error("3 * 4 + 1 - 3 ; #1 (#1 / 2) ;",[10,5])
test_rattrapage_error("3 * / 1 - 3 ; #1 * (#1 / 2) ;",[0,0])
test_rattrapage_error("3 * 4 + 1 - 3 #1 * (#1 / 2) ;",[11.5])
test_rattrapage_error("(1 2 ;",[1])
test_rattrapage_error("- ((1 + 2 * - ((3 - 5))) ; ",[3])
test_rattrapage_error("- (1 + 2)) * - ((3 - 5)) ; ",[-6])
test_rattrapage_error("!5;",[5])
test_rattrapage_error("5! / ;",[120])
